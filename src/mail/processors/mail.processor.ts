    ...
    try {
      switch (job.name) {
        ...
        case 'sendVerificationLink':
          ...
          break;
        case 'sendPractitionerCredentials':
          this.logger.log('Sending practitioner credentials email');
          await this.mailerService.sendMail({
            to: job.data.to,
            from: job.data.from,
            subject: job.data.subject,
          ...
          break;
        case 'sendPrescriptionAssignedEmail':
          this.logger.log('Sending prescription assigned email');
          await this.mailerService.sendMail({
            to: job.data.to,
            from: job.data.from,
            subject: job.data.subject,
